...
%rmdir(temp_dir,'s')
  ...
  % copyfile(fullfile('unused', [imagefilter, '*.hdr*']), temp_dir)
   %copyfile(fullfile('unused', [imagefilter, '*.img*']), temp_dir)
    end
  end
  cd(temp_dir);
   unix('gunzip -fq *');
...
% wm_csf_ts_org=load(fullfile(mvmntdir{FCi,2}, 'compcor.mat'));
% wm_csf_ts=wm_csf_ts_org.nui.tc

% if bandpass_on == 1
%     wm_csf_ts = bandpass_final_SPM_ts(TR_val, fl, fh, wm_csf_ts);
% end
% wm_csf_ts=wm_csf_ts(NUMTRUNC(1)+1:end-NUMTRUNC(2), :);
% 
...
%       diary off; return;
%     end
 rp2 = dir(fullfile(mvmntdir{FCi,2}, 'friston24*.txt'));
mvmnt=load(fullfile(mvmntdir{FCi,2}, rp2(1).name));
...
%-Demeaned ROI timeseries and wm+csf signals
    ...
    reg_ts_all = fullfile(reg_ts_dir, 'roi_wm_csf_mvmnt_Filtered.txt');
