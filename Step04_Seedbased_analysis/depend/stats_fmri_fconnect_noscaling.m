...
function stats_fmri_fconnect_noscaling (datadir, data_type, NUMTRUNC, imagefilter, scrdir, rsTR)
...
load(fullfile(scrdir, 'depend/batch_stats_spm12.mat'));

% Update TR
matlabbatch{1}.spm.stats.fmri_spec.timing.RT = rsTR; 
...
for filecnt = (NUMTRUNC(1)+1):(nscans-NUMTRUNC(2))
  ...
  matlabbatch{1}.spm.stats.fmri_spec.sess(1).scans{filecnt,1} = ...
...
load(fullfile(scrdir, 'depend/rsFC_contrasts.mat'));
